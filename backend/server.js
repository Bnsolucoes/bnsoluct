...
const fs = require("fs"); // Importar o módulo 'fs' para leitura de arquivos
...
// Carregar o prompt do chatbot do arquivo
let chatbotSystemPrompt = "";
try {
  chatbotSystemPrompt = fs.readFileSync(path.join(__dirname, "chatbot_prompt.md"), "utf8");
  console.log("Prompt do chatbot carregado com sucesso!");
} catch (error) {
  console.error("Erro ao carregar o prompt do chatbot:", error);
  // Fallback para um prompt padrão ou tratamento de erro
  chatbotSystemPrompt = "Você é um assistente prestativo."; 
}
...
const chatHandler = async (req, res) => {
  ...
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: chatbotSystemPrompt }, // Usando o prompt carregado do arquivo
